...
// learner+creator pair in school_subscriptions), but that one row unlocks
// only ONE course at a time (school_subscriptions.course_id) — not every
// course the creator publishes. Subscribing to a different course from the
// same creator switches which course_id that single row points at; the
// previously-unlocked course locks again immediately (see
// apply_school_subscription_payment_success()).
//
...
/**
 * True when $learnerId currently has paid, unexpired access to $courseId
 * specifically — not to every course $creatorId publishes. Computes the
 * grace deadline directly from current_period_ends_at +
 * SCHOOL_SUBSCRIPTION_GRACE_DAYS rather than trusting the stored
 * status/grace_ends_at columns — access control stays correct even if
 * cron/track-maintenance.php's sweep hasn't run recently; that sweep only
 * keeps status current for display (the learner's subscriptions list,
 * admin reporting), it isn't load-bearing for access.
 */
function learner_has_active_school_subscription(int $learnerId, int $creatorId, int $courseId): bool {
    $row = db_one(
        "SELECT current_period_ends_at FROM school_subscriptions
         WHERE learner_id = ? AND creator_id = ? AND course_id = ? AND status IN ('ACTIVE','GRACE') LIMIT 1",
        [$learnerId, $creatorId, $courseId]
    );
    if (!$row) return false;
...
/** Every school subscription a learner holds, newest-period-first, with the creator's name/school branding and the unlocked course's title joined on. */
 ...
 * subscription, scoped to one course ($courseId) — success unlocks just
 * that course, switching it in for whichever course this learner's
 * subscription to this creator previously pointed at, if any. Logged-in
 * learners only — unlike a free course, a paid subscription needs an
 * account, so there's no guest flow here.
 * @return array{paymentId?: int, error?: string}
 */
function initiate_school_subscription(int $learnerId, int $creatorId, int $courseId, string $phone): array {
    ...
    if (learner_has_active_school_subscription($learnerId, $creatorId, $courseId)) {
        return ['error' => "You're already subscribed to this course."];
    }

    $existingPending = db_one(
        "SELECT id FROM payments WHERE user_id = ? AND school_subscription_creator_id = ? AND course_id = ? AND type = 'SCHOOL_SUBSCRIPTION' AND status = 'PENDING' ORDER BY created_at DESC LIMIT 1",
        [$learnerId, $creatorId, $courseId]
    );
    if ($existingPending) {
        $resolved = resolve_payment_with_iotec(fetch_payment_with_school_subscription((int) $existingPending['id']));
        if ($resolved['status'] === 'SUCCESS') return ['error' => "You're already subscribed to this course."];
    ...
    $existingSub = get_school_subscription($learnerId, $creatorId); // may be for a different course, or EXPIRED/CANCELED — fine, success repoints/renews it

    $paymentId = db_insert(
        "INSERT INTO payments (user_id, school_subscription_id, school_subscription_creator_id, course_id, amount, phone, type, status) VALUES (?, ?, ?, ?, ?, ?, 'SCHOOL_SUBSCRIPTION', 'PENDING')",
        [$learnerId, $existingSub ? $existingSub['id'] : null, $creatorId, $courseId, $price, $phone]
    );
 ...
 * subscription row. Sets/overwrites course_id to $payment['course_id'] —
 * a learner switching to a different course from the same creator repoints
 * their one subscription row rather than adding a second active course, so
 * whatever they had unlocked before locks again immediately (the live
 * check in learner_has_active_school_subscription() now requires course_id
 * to match). Called only from resolve_payment_with_iotec().
 */
function apply_school_subscription_payment_success(array $payment): void {
    ...
    $courseId = (int) $payment['course_id'];
    ...
    try {
        ...
        if ($existing) {
            ...
            db_run(
                ...
                [$payment['amount'], $payment['phone'], $courseId, $periodEndsAt, $subscriptionId]
            );
        } else {
            $subscriptionId = db_insert(
                ...
                [$payment['amount'], $payment['phone'], $periodEndsAt, $learnerId, $creatorId, $courseId]
            );
        }
        ...
        db_insert(
            ...
            [$creatorId, $courseId, $split['net'], $split['gross'], $split['fee']]
        );
        db()->commit();
    } catch (Throwable $e) {
        db()->rollBack();
...
function send_due_school_subscription_renewal_reminders(): int {
    $due = db_all(
        "SELECT ss.id, ss.creator_id, u.name AS creator_name, u.school_name,
                learner.name AS learner_name, learner.email AS learner_email, c.title AS course_title
         FROM school_subscriptions ss
         JOIN users u ON u.id = ss.creator_id
    ...
    foreach ($due as $sub) {
        ...
        send_school_subscription_renewal_email($sub['learner_email'], $sub['learner_name'], $schoolLabel, $sub['course_title'] ?: 'your course', $renewUrl);
