...
$perPage = 30;
$result = get_course_shares($filters, $page, $perPage, (int) $user['id']);
$shares = $result['rows'];
$totalShares = $result['total'];
$totalPages = max(1, (int) ceil($totalShares / $perPage));

$summary = get_share_summary(30, (int) $user['id']);
$topCourses = get_top_shared_courses(30, 8, (int) $user['id']);
...
<div class="chart-card" style="margin-top:14px; padding:18px 20px;">
  <form method="get" class="leads-filter-bar">
    ...
    <?php if (array_filter($filters)): ?><a href="<?= e(base_url('dashboard/creator/shares.php')) ?>" class="small muted">Clear</a><?php endif; ?>
...
<?php if ($totalPages > 1): ?>
  <div class="row gap-2" style="margin-top:16px; justify-content:center;">
    <?php for ($p = 1; $p <= $totalPages; $p++): ?>
      <a href="<?= e(base_url('dashboard/creator/shares.php?' . http_build_query(array_filter($filters) + ['page' => $p]))) ?>"
