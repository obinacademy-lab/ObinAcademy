...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $action = post('_action');
    $app = db_one('SELECT a.*, u.name AS applicant_name, u.email AS applicant_email FROM affiliate_applications a JOIN users u ON u.id=a.user_id WHERE a.id=?', [$id]);

    if ($app && $app['status'] === 'PENDING' && $action === 'approve') {
        $affiliate = approve_affiliate_application($id);
        log_admin_action((int) $user['id'], $user['name'], 'affiliate_application.approved', 'User', $app['applicant_name'], 'ref_code ' . $affiliate['ref_code']);
        send_affiliate_application_approved_email($app['applicant_email'], $app['applicant_name'], $affiliate['ref_code']);
    } elseif ($app && $app['status'] === 'PENDING' && $action === 'reject') {
        reject_affiliate_application($id, post('rejectionReason'));
        log_admin_action((int) $user['id'], $user['name'], 'affiliate_application.rejected', 'User', $app['applicant_name']);
    }
    redirect('/dashboard/admin/affiliate-applications.php');
}

$pending = db_all("SELECT a.*, u.name, u.email FROM affiliate_applications a JOIN users u ON u.id=a.user_id WHERE a.status='PENDING' ORDER BY a.created_at ASC");
$resolved = db_all("SELECT a.*, u.name, u.email FROM affiliate_applications a JOIN users u ON u.id=a.user_id WHERE a.status!='PENDING' ORDER BY a.reviewed_at DESC LIMIT 30");

$approvedTotal = (int) db_one("SELECT COUNT(*) AS n FROM affiliate_applications WHERE status='APPROVED'")['n'];
$rejectedTotal = (int) db_one("SELECT COUNT(*) AS n FROM affiliate_applications WHERE status='REJECTED'")['n'];
$decidedTotal = $approvedTotal + $rejectedTotal;
$approvalRate = $decidedTotal > 0 ? round(($approvedTotal / $decidedTotal) * 100) : null;

$pageTitle = 'Affiliate Applications — Admin — Obin Academy';
...
<?php if (!$pending): ?>
  <div class="all-caught-up" style="margin-top:14px;">
    ...
    <div><strong>Nothing pending.</strong> Every affiliate application has been reviewed.</div>
  </div>
<?php else: ?>
    ...
    <?php foreach ($pending as $a): $daysWaiting = (int) floor((time() - strtotime($a['created_at'])) / 86400); ?>
      <div class="application-card">
        ...
        <div class="application-body">
          <div class="application-field"><span class="label">How they plan to share Obin Academy</span><p><?= e($a['motivation']) ?></p></div>
        </div>
